...
文書を書く・直すときは [CONTEXT.md](CONTEXT.md) の用語を使い、各用語の _Avoid_ に挙げた表記を使わない。公式用語は公式サイトで確認した名称、プロジェクト定義用語はこのプロジェクトが付けた名前で、CONTEXT.md がその区別を示す。新しい概念に名前を付けるときや、用語の揺れに気づいたときは、CONTEXT.md を先に直してから文書を直す。プレイヤーの俗語は CONTEXT.md の用語へ対応付ける用語集を別に整備する([Issue 12](https://github.com/mokosystem/pokemon-tcg-buddy/issues/12))。
